...
class ConnectionChallenge(models.Model):
    connection_challenge = models.CharField(
        primary_key=True, 
        max_length=128,
        help_text="""A random challenge string that is independently calculated by both the client and server.
        Traditionally, this is calculated as SHA-512 of a the Server's public key + a random shared secret generated by the client."""
    )
    account = models.ForeignKey(Account, related_name="connection_challenge", on_delete=models.CASCADE)
    created_at = models.DateTimeField(auto_now_add=True)

    def __str__(self):
        return f"Connection challenge for {self.account} created at {self.created_at}"
